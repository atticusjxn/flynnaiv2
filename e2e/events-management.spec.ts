...
test.describe('Events Management', () => {
  ...
  test('should allow editing event details', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="event-title"]')).toContainText(
      'Updated Kitchen Repair'
    );
    await expect(page.locator('[data-testid="urgency-badge"]')).toContainText(
      'High'
    );
  });
  ...
  test('should handle bulk actions on events', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="event-status"]').first()
  ...
  test('should sync events to calendar', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="calendar-sync-dialog"]')
    ).toBeVisible();
    ...
    await expect(page.locator('[data-testid="calendar-status"]')).toContainText(
      'Synced'
    );
  });
  ...
  test('should handle event conflicts', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="conflict-warning"]')
    ).toContainText('Schedule conflict detected');
  ...
  test('should send customer notifications', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="notification-dialog"]')
    ).toBeVisible();
    ...
    ).toContainText('Notification sent successfully');
  });
  ...
  test('should show event analytics and metrics', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="urgency-breakdown"]')
    ).toBeVisible();

    // Check metrics values
    await expect(page.locator('[data-testid="total-events"]')).toContainText(
      /\d+/
    );
    await expect(
      page.locator('[data-testid="confirmed-events"]')
    ).toContainText(/\d+/);
    await expect(page.locator('[data-testid="pending-events"]')).toContainText(
      /\d+/
    );
  });
