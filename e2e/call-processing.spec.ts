...
test.describe('Call Processing End-to-End', () => {
  ...
  test('should process plumbing call from start to finish', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="call-item"]').first()).toBeVisible({ timeout: 10000 })
  ...
  test('should handle emergency calls with high priority', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="urgency-badge"]').first()).toContainText('Emergency')
    await expect(page.locator('[data-testid="event-card"]').first()).toHaveClass(/.*emergency.*/)
  })
  ...
  test('should show real-time call notifications', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="live-notification"]')).toBeVisible({ timeout: 5000 })
    await expect(page.locator('[data-testid="live-notification"]')).toContainText('Incoming call')
  })
