        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex items-center space-x-4 text-xs">
            <div className={cn(
              ...
              industry.requiresLocation ? 'text-green-600' : 'text-muted-foreground'
            )}>
            ...
            <div className={cn(
              'flex items-center space-x-1',
              industry.pricingExpected ? 'text-blue-600' : 'text-muted-foreground'
            )}>
              <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
...
export default function IndustrySelector({ currentIndustry, onIndustryChange }: IndustrySelectorProps) {
  const industries = getAllIndustries();
  const [selectedIndustry, setSelectedIndustry] = useState<IndustryConfiguration | null>(currentIndustry);
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Choose the industry that best matches your business to optimize Flynn.ai for your specific needs. 
          This will customize terminology, event types, and workflow patterns.
        </p>
      </div>
          ...
          >
            <div className="flex items-center space-x-4">
              <div 
                ...
                style={{ backgroundColor: selectedIndustry.colors.primary + '20' }}
              >
                <div 
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Flynn.ai will be optimized for {selectedIndustry.name.toLowerCase()} workflows and terminology
                </p>
              </div>
      ...
      {selectedIndustry && (
        ...
        >
          <h5 className="font-medium text-foreground mb-3">Business Configuration Preview</h5>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div className="space-y-1">
              <span className="text-muted-foreground">Operating Hours</span>
              <p className="font-medium text-foreground">{selectedIndustry.businessHours}</p>
            </div>
            <div className="space-y-1">
              <span className="text-muted-foreground">Appointment Term</span>
              <p className="font-medium text-foreground capitalize">{selectedIndustry.terminology.appointment}</p>
            </div>
            <div className="space-y-1">
              <span className="text-muted-foreground">Customer Term</span>
              <p className="font-medium text-foreground capitalize">{selectedIndustry.terminology.customer}</p>
            </div>
          </div>
        </motion.div>
      )}
