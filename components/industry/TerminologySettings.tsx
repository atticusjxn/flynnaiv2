    ...
    >
      ...
      <div className="space-y-3 text-sm">
        <div className="bg-muted/30 rounded-lg p-4">
          ...
          <p className="text-muted-foreground">
            {terminology.summary_title || 'Call Summary'} - New{' '}
            {terminology.appointment || 'appointment'} scheduled
          </p>
        </div>
        ...
        <div className="bg-muted/30 rounded-lg p-4">
          ...
          <div className="space-y-2 text-muted-foreground">
            <p>
              Your {terminology.customer || 'customer'} John Smith called to
              schedule a {terminology.appointment || 'appointment'}.
            </p>
            <p>
              <strong>{terminology.events_title || 'Events'}:</strong>
            </p>
            <p className="ml-4">
              • {terminology.service_call || 'Service Call'} - Tomorrow at 2:00
              PM
            </p>
            {terminology.quote && (
              <p className="ml-4">
      ...
      <div className="pt-4 border-t border-border">
        <p className="text-xs text-muted-foreground">
          Changes are applied in real-time to help you see how they'll appear to
          your customers
        </p>
      </div>
    </motion.div>
...
export default function TerminologySettings({
  ...
  const [currentTerminology, setCurrentTerminology] = useState<
  ...
  const categories = [
    { id: 'all', label: 'All Terms', count: terminologyItems.length },
    {
      id: 'core',
      label: 'Core Terms',
      count: terminologyItems.filter((i) => i.category === 'core').length,
    },
    {
      id: 'events',
      label: 'Event Types',
      count: terminologyItems.filter((i) => i.category === 'events').length,
    },
    {
      id: 'customer',
      label: 'Customer Terms',
      count: terminologyItems.filter((i) => i.category === 'customer').length,
    },
  ];

  const hasChanges = terminologyItems.some(
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Customize the language Flynn.ai uses to match your business
            communication style
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1">
          <PreviewCard terminology={currentTerminology} industry={industry} />
        </div>
      </div>
          ...
          >
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h4 className="font-medium text-blue-800">
                {
                  terminologyItems.filter((i) => i.value !== i.originalValue)
                    .length
                }{' '}
                Changes Pending
              </h4>
              ...
              {terminologyItems
                ...
                .map((item) => (
                  <div key={item.key} className="flex items-center space-x-2">
                    <span>•</span>
                    <span>
                      {item.label}: "{item.originalValue}" → "{item.value}"
                    </span>
                  </div>
                ))}
            </div>
