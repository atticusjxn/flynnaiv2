...
export default function UsageMetrics({ period = 'month' }: UsageMetricsProps) {
  ...
  const usagePercentage = isUnlimited
    ? 0
    : (analytics.usage.calls.used / analytics.usage.calls.limit) * 100;
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          ...
          >
            {isUnlimited
              ? 'Unlimited'
              : `${analytics.usage.calls.remaining} calls left`}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="text-2xl font-bold text-foreground">
                {analytics.usage.calls.used.toLocaleString()}
              </div>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="text-2xl font-bold text-foreground">
                {analytics.usage.events.extracted.toLocaleString()}
              </div>
              <div className="text-xs text-muted-foreground">
                Events Created
              </div>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="text-2xl font-bold text-foreground">
                {Math.round(analytics.usage.events.successRate)}%
              </div>
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-950/20 dark:to-gray-900/30 rounded-lg p-4 mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div
            className={`flex items-center gap-2 ${analytics.features.has_calendar_sync ? 'text-green-600' : 'text-gray-400'}`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div
            className={`flex items-center gap-2 ${analytics.features.has_sms_notifications ? 'text-green-600' : 'text-gray-400'}`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div
            className={`flex items-center gap-2 ${analytics.features.has_bulk_management ? 'text-green-600' : 'text-gray-400'}`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div
            className={`flex items-center gap-2 ${analytics.features.has_api_access ? 'text-green-600' : 'text-gray-400'}`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-foreground mb-1">
              Australian Business Value
            </h3>
            <p className="text-sm text-muted-foreground">
              Estimated time saved:{' '}
              <span className="font-semibold text-foreground">
                ~{Math.round(analytics.usage.calls.used * 5)} minutes
              </span>
            </p>
          </div>
          <div className="text-right">
