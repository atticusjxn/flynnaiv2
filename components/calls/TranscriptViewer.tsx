...
const parseTranscript = (transcriptionText: string): TranscriptSegment[] => {
  ...
  return paragraphs.map((text, index) => ({
    ...
    text: text.trim(),
    timestamp: `00:${Math.floor((index * 30) / 60)
      .toString()
      .padStart(2, '0')}:${((index * 30) % 60).toString().padStart(2, '0')}`,
...
const highlightKeyInfo = (text: string): string => {
  ...
  patterns.forEach(({ regex, class: className }) => {
    highlightedText = highlightedText.replace(
      regex,
      `<span class="px-1 py-0.5 rounded text-xs font-medium ${className}">$1</span>`
    );
  });
...
export default function TranscriptViewer({
  ...
  const [highlightMode, setHighlightMode] = useState<
    'none' | 'keywords' | 'confidence'
  >('keywords');

  const segments = useMemo(
    () => parseTranscript(call.transcription_text || ''),
      ...
      <Card className="w-full">
        <CardBody className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            This call doesn't have a transcript yet. It may still be processing
            or the call might not have been recorded.
          </p>
        </CardBody>
      </Card>
    );
        ...
        <div className="flex items-center justify-between w-full">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Call with {call.caller_name || call.caller_number} •{' '}
              {call.call_duration
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full">
          ...
          <div className="flex gap-2">
            <Button
              ...
              onPress={() =>
                setHighlightMode(
                  highlightMode === 'keywords' ? 'none' : 'keywords'
                )
              }
              aria-label={
            ...
            <Button
              ...
              onPress={() =>
                setHighlightMode(
                  highlightMode === 'confidence' ? 'none' : 'confidence'
                )
              }
              aria-label={
            ...
            filteredSegments.map((segment, index) => {
              ...
              return (
                ...
                >
                  {/* Speaker Avatar */}
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-${speakerInfo.color}/10 border-2 border-${speakerInfo.color}/20`}
                  >
