...
interface CalendarSettingsProps {
  ...
  onUpdate: (
    section: 'calendar',
    updates: Partial<UserSettings['calendar']>
  ) => void;
  hasChanges: boolean;
  onSave: () => void;
  isSaving: boolean;
...
const conflictResolutionOptions = [
  {
    value: 'manual',
    label: 'Manual Review',
    description: 'I will manually resolve any calendar conflicts',
  },
  {
    value: 'auto_reschedule',
    label: 'Auto Reschedule',
    description: 'Automatically reschedule conflicting events to nearby times',
  },
  {
    value: 'skip_conflicts',
    label: 'Skip Conflicts',
    description: "Don't create calendar events when conflicts are detected",
  },
];
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Connect and manage your calendar providers for automatic event
            synchronization
          </p>
        </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Automatically create calendar events from extracted appointments
              </div>
            </div>
            <input
              type="checkbox"
          ...
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              <div className="font-medium text-foreground">
                Sync Only Confirmed Events
              </div>
              <div className="text-sm text-muted-foreground">
                Only sync events after they have been manually confirmed
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('syncOnlyConfirmed', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
          ...
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Create calendar events even if they overlap with existing ones
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('allowOverlappingEvents', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
          ...
          {conflictResolutionOptions.map((option) => (
            ...
            >
              <input
                ...
                onChange={(e) =>
                  handleChange('conflictResolution', e.target.value)
                }
          ...
          {[
            {
              ...
              description:
                'Include customer contact details in event description',
            },
            {
              key: 'includeCallTranscript',
              label: 'Call Transcript',
              description: 'Attach call transcript to calendar event',
            },
            {
              key: 'includeLocation',
              label: 'Location Details',
              description: 'Add location/address information when available',
            },
            {
              key: 'privateEvents',
              label: 'Private Events',
              description:
                'Mark all Flynn.ai events as private in your calendar',
            },
          ].map((item) => (
            <label
              key={item.key}
            ...
            >
              <input
                ...
                onChange={(e) =>
                  handleChange(
                    item.key as keyof UserSettings['calendar'],
                    e.target.checked
                  )
                }
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Add reminder notifications to calendar events
              </div>
            </div>
            <input
              type="checkbox"
            ...
            <div className="bg-muted/20 rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {reminderOptions.map((reminder) => {
                  const isSelected = (
                    localSettings.reminderMinutes || []
                  ).includes(reminder.value);
                  return (
                    <label
                      key={reminder.value}
              ...
              <p className="text-xs text-muted-foreground">
                Select multiple reminder times. Events will have all selected
                reminders.
              </p>
            </div>
          )}
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Create events in your configured business timezone
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('useBusinessTimezone', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
          ...
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Automatically convert event times to your local timezone
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('convertToLocalTime', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
