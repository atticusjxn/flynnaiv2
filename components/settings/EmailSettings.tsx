...
const frequencyOptions = [
  {
    ...
    label: 'Immediate',
    description:
      'Send emails as soon as calls are processed (within 2 minutes)',
  },
  {
    value: 'hourly',
    label: 'Hourly Digest',
    description: 'Bundle all new calls into hourly summaries',
  },
  {
    value: 'daily',
    label: 'Daily Digest',
    description: 'One summary email per day with all calls',
  },
  {
    value: 'weekly',
    label: 'Weekly Digest',
    description: 'Weekly summary of all call activity',
  },
];
...
const templateStyles = [
  {
    value: 'professional',
    label: 'Professional',
    description: 'Clean, business-focused layout with minimal branding',
  },
  {
    value: 'modern',
    label: 'Modern',
    description: 'Contemporary design with visual elements and icons',
  },
  {
    value: 'minimal',
    label: 'Minimal',
    description: 'Simple text-based format with essential information only',
  },
  {
    value: 'detailed',
    label: 'Detailed',
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Choose how often you receive email notifications about processed
            calls
          </p>
        </div>
          ...
          <div className="bg-muted/20 rounded-lg p-4 space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {localSettings.frequency === 'daily' ? 'Daily' : 'Weekly'}{' '}
                emails will be sent at this time
              </p>
            </div>
            ...
            {localSettings.frequency === 'weekly' && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleChange('weeklyDigestDay', e.target.value)
                  }
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            {
              key: 'includeEventDetails',
              label: 'Event Details',
              description: 'Extracted appointments and meetings',
            },
            {
              key: 'includeCustomerInfo',
              label: 'Customer Information',
              description: 'Contact details and customer notes',
            },
            {
            ...
            {
              key: 'includeRecordingLink',
              label: 'Recording Link',
              description: 'Link to access the call recording',
            },
            {
              key: 'autoAttachICS',
              label: 'Calendar Files (.ics)',
              description: 'Attach calendar files for easy import',
            },
          ].map((item) => (
            <label
              key={item.key}
              className="flex items-start space-x-3 p-3 border border-border rounded-lg cursor-pointer hover:bg-muted/20 transition-colors"
            >
              <input
                type="checkbox"
                checked={
                  localSettings[
                    item.key as keyof typeof localSettings
                  ] as boolean
                }
                onChange={(e) =>
                  handleChange(
                    item.key as keyof UserSettings['email'],
                    e.target.checked
                  )
                }
            ...
            {templateStyles.map((style) => (
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    handleChange('templateStyle', e.target.value)
                  }
                  className="w-4 h-4 text-primary border-border focus:ring-primary focus:ring-2 mt-1"
                />
                <div>
          ...
          <div className="space-y-3">
            <label className="flex items-center justify-between p-3 border border-border rounded-lg">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Include Flynn.ai logo and branding in emails
                </div>
              </div>
              <input
                type="checkbox"
                ...
                onChange={(e) =>
                  handleChange('brandingEnabled', e.target.checked)
                }
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleChange('customSignature', e.target.value)
                }
              ...
              <p className="text-xs text-muted-foreground">
                This signature will be added to the end of all email
                notifications
              </p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Only send emails during your configured business hours
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('businessHoursOnly', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
          ...
          <div className="border border-border rounded-lg p-4 space-y-4">
            <label className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  Don't send emails during specified quiet hours
                </div>
              </div>
              <input
                type="checkbox"
          ...
          <label className="flex items-center justify-between p-4 border border-border rounded-lg">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Include links to confirm, edit, or reschedule events
              </div>
            </div>
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                handleChange('includeManageEventLinks', e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-primary focus:ring-2"
            />
          </label>
