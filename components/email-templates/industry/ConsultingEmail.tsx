...
export default function ConsultingEmail(props: ConsultingEmailProps) {
  const hasHighValueOpportunity = props.extractedEvents.some(e => e.urgency === 'high');
    ...
    <CallOverviewEmail {...props}>
      ...
      {hasHighValueOpportunity && (
        <Section style={highValueSection}>
          <Text style={highValueTitle}>
            💎 HIGH-VALUE OPPORTUNITY
          </Text>
          <Text style={highValueText}>
            This consultation indicates a high-value strategic engagement opportunity. 
            Complex business challenges typically require comprehensive solutions and 
        ...
        <Row>
          <Column>
            ...
            <Text style={challengeText}>
              {categorizeBusinessChallenge(props.extractedEvents, props.transcriptionSnippet)}
            </Text>
          </Column>
          <Column>
      ...
      <Section style={opportunitySection}>
        <Text style={opportunityTitle}>💼 Consulting Opportunity Assessment</Text>
        <Row>
          <Column>
            <Text style={opportunityLabel}>Engagement Type:</Text>
            <Text style={opportunityText}>
              {determineEngagementType(props.extractedEvents, props.transcriptionSnippet)}
            </Text>
          </Column>
          <Column>
      ...
      <Section style={methodologySection}>
        ...
        <Text style={methodologyText}>
          Based on the consultation discussion, here's the recommended methodology:
        </Text>
        
        <Text style={phaseTitle}>Phase 1: Discovery & Assessment</Text>
        <Text style={phaseText}>
          • Current state analysis and stakeholder interviews<br />
          • Process mapping and pain point identification<br />
          • Competitive benchmarking and industry best practices
        </Text>
        
        <Text style={phaseTitle}>Phase 2: Strategy Development</Text>
        <Text style={phaseText}>
          • Solution design and strategic recommendations<br />
          • Implementation roadmap with milestones<br />
        ...
        <Text style={phaseText}>
          ...
          • Progress monitoring and course correction<br />
      ...
      <Section style={competitiveSection}>
        ...
        <Text style={differentiatorText}>
          • Deep industry expertise and proven methodologies<br />
          • Data-driven approach with measurable outcomes<br />
          • Senior consultant involvement throughout engagement<br />
          • Flexible engagement models to fit client needs<br />
      ...
      <Section style={bestPracticesSection}>
        <Text style={bestPracticesTitle}>🎓 Consulting Excellence Framework:</Text>
        <Text style={bestPracticesList}>
          • <strong>Discovery First:</strong> Understand the real problem before proposing solutions<br />
          • <strong>Value-Based Pricing:</strong> Price based on business impact, not time<br />
          • <strong>Executive Sponsorship:</strong> Ensure C-level buy-in for strategic initiatives<br />
          • <strong>Change Management:</strong> Address people and process alongside technology<br />
          • <strong>Measurable Outcomes:</strong> Define success metrics upfront<br />
          • <strong>Knowledge Transfer:</strong> Build internal capability for sustainability
        </Text>
      </Section>
      ...
      <Section style={standardsSection}>
        ...
        <Text style={standardsText}>
          • All engagements follow established consulting methodologies<br />
          • Client confidentiality and non-disclosure agreements strictly enforced<br />
          • Conflict of interest screening for all new engagements<br />
          • Quality assurance and peer review processes<br />
          • Professional liability insurance and ethical standards compliance
          {props.consultantName && `<br />• Lead Consultant: ${props.consultantName}`}
          {props.specialization && `<br />• Specializations: ${props.specialization.join(', ')}`}
        </Text>
      </Section>
    </CallOverviewEmail>
  );
...
function categorizeBusinessChallenge(events: any[], transcription?: string): string {
  const content = transcription?.toLowerCase() || '';
  
  // Strategy & Planning
  if (content.includes('strategy') || content.includes('planning') || content.includes('roadmap') ||
      content.includes('vision') || content.includes('competitive')) {
    return 'Strategic Planning & Competitive Positioning';
  }
  
  // Operations & Process
  if (content.includes('process') || content.includes('operations') || content.includes('efficiency') ||
      content.includes('workflow') || content.includes('optimization')) {
    return 'Operational Excellence & Process Optimization';
  }
  
  // Technology & Digital
  if (content.includes('technology') || content.includes('digital') || content.includes('automation') ||
      content.includes('software') || content.includes('system')) {
    return 'Digital Transformation & Technology Implementation';
  }
  
  // Organizational & Change
  if (content.includes('organization') || content.includes('culture') || content.includes('change') ||
      content.includes('leadership') || content.includes('team')) {
  ...
  if (content.includes('financial') || content.includes('cost') || content.includes('revenue') ||
      content.includes('profit') || content.includes('performance')) {
    return 'Financial Performance & Cost Management';
  }
  
  // Growth & Market
  if (content.includes('growth') || content.includes('market') || content.includes('expansion') ||
      content.includes('acquisition') || content.includes('merger')) {
...
function assessComplexity(events: any[], transcription?: string): string {
  ...
  if (content.includes('enterprise') || content.includes('global') || content.includes('multi-national')) complexityScore += 3;
  if (content.includes('transformation') || content.includes('restructure')) complexityScore += 3;
  if (content.includes('multiple departments') || content.includes('cross-functional')) complexityScore += 2;
  if (content.includes('regulatory') || content.includes('compliance')) complexityScore += 2;
  if (content.includes('integration') || content.includes('merger')) complexityScore += 2;
  
  // Medium complexity indicators
  if (content.includes('process improvement') || content.includes('optimization')) complexityScore += 1;
  if (content.includes('team') || content.includes('department')) complexityScore += 1;
  
  if (complexityScore >= 6) return 'Enterprise - Large-scale organizational transformation';
  if (complexityScore >= 4) return 'High - Complex multi-stakeholder engagement';
...
function assessStrategicImpact(transcription?: string): string {
  ...
  if (content.includes('mission critical') || content.includes('strategic priority') ||
      content.includes('company future') || content.includes('survival')) {
    return 'Mission Critical - Core business impact';
  }
  
  if (content.includes('competitive advantage') || content.includes('market position') ||
      content.includes('significant impact')) {
    return 'High Strategic Value - Competitive differentiation';
  }
  
  if (content.includes('efficiency gains') || content.includes('cost savings') ||
...
function identifyStakeholders(transcription?: string): string {
  ...
  if (content.includes('ceo') || content.includes('president') || content.includes('owner')) {
  ...
  if (content.includes('department heads') || content.includes('directors') || content.includes('vp')) {
  ...
  if (content.includes('employees') || content.includes('staff') || content.includes('workforce')) {
...
function determineEngagementType(events: any[], transcription?: string): string {
  const content = transcription?.toLowerCase() || '';
  
  if (content.includes('ongoing') || content.includes('retained') || content.includes('long term')) {
    return 'Retained Consulting - Long-term partnership';
  }
  
  if (content.includes('project') || content.includes('specific deliverable') || content.includes('fixed scope')) {
  ...
  if (content.includes('workshop') || content.includes('training') || content.includes('facilitation')) {
    return 'Facilitation & Training - Knowledge transfer focus';
  }
  
  if (content.includes('interim') || content.includes('temporary') || content.includes('fractional')) {
...
function estimateEngagementDuration(transcription?: string): string {
  ...
  if (content.includes('quick') || content.includes('urgent') || content.includes('immediate')) {
    return '2-4 weeks - Sprint engagement';
  }
  
  if (content.includes('this quarter') || content.includes('few months') || content.includes('short term')) {
    return '1-3 months - Tactical project';
  }
  
  if (content.includes('transformation') || content.includes('major initiative') || content.includes('comprehensive')) {
    return '6-12 months - Strategic transformation';
  }
  
  if (content.includes('ongoing') || content.includes('retained') || content.includes('partnership')) {
...
function extractBudgetSignals(transcription?: string): string {
  ...
  if (content.includes('significant investment') || content.includes('major initiative') ||
      content.includes('substantial budget')) {
    return 'High Budget Allocation - Major investment approved';
  }
  
  if (content.includes('budget allocated') || content.includes('funding secured') ||
      content.includes('approved spend')) {
    return 'Budget Confirmed - Funding in place';
  }
  
  if (content.includes('cost conscious') || content.includes('tight budget') ||
...
function calculateBusinessImpact(transcription?: string): string {
  ...
  if (content.includes('millions') || content.includes('enterprise wide') || 
      content.includes('company transformation')) {
    return 'High Impact - Multi-million dollar opportunity';
  }
  
  if (content.includes('department') || content.includes('significant improvement') ||
...
function estimateROIPotential(transcription?: string): string {
  ...
  if (content.includes('cost reduction') || content.includes('efficiency') ||
      content.includes('automation')) {
    return '3-5x ROI - Cost reduction focus';
  }
  
  if (content.includes('revenue growth') || content.includes('market expansion') ||
...
function identifyValueDrivers(transcription?: string): string {
  ...
  if (content.includes('cost') || content.includes('efficiency')) drivers.push('Cost reduction and efficiency gains');
  if (content.includes('revenue') || content.includes('growth')) drivers.push('Revenue growth and market expansion');
  if (content.includes('risk') || content.includes('compliance')) drivers.push('Risk mitigation and compliance');
  if (content.includes('speed') || content.includes('agility')) drivers.push('Improved agility and time-to-market');
  if (content.includes('quality') || content.includes('excellence')) drivers.push('Quality improvement and operational excellence');
  
  if (drivers.length === 0) return 'Value drivers to be identified during discovery';
...
function getCompetitivePositioning(transcription?: string): string {
  ...
  if (content.includes('big four') || content.includes('mckinsey') || content.includes('bain') ||
      content.includes('deloitte') || content.includes('pwc')) {
    return 'Competing with tier-1 consulting firms - Emphasize specialized expertise and partner-level attention';
  }
  
  if (content.includes('boutique') || content.includes('specialist') || content.includes('niche')) {
    return 'Boutique consulting landscape - Highlight deep expertise and personalized service';
  }
  
  if (content.includes('internal team') || content.includes('in-house') || content.includes('build vs buy')) {
