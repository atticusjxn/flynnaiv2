...
export const AustralianPhoneNumberSchema = z.string().refine((phone) => {
  ...
  if (cleaned.startsWith('61') && cleaned.length === 12) {
    ...
    return /^[2378]\d{8}$/.test(nationalNumber) || /^4\d{8}$/.test(nationalNumber);
  }
...
export const UserRegistrationSchema = z.object({
  ...
  industry: z.enum(['plumbing', 'real_estate', 'legal', 'medical', 'sales', 'consulting', 'other']),
...
export const UserUpdateSchema = z.object({
  ...
  industry: z.enum(['plumbing', 'real_estate', 'legal', 'medical', 'sales', 'consulting', 'other']).optional(),
...
export const RateLimitSchema = z.object({
  ...
  endpoint: z.enum(['ai-processing', 'webhooks', 'user-settings', 'onboarding']),
