...
export class SMSService {
  ...
  async sendSMS(options: SMSOptions): Promise<{
    ...
    try {
      ...
      const messageContent = this.generateMessageContent(options.template, options.templateData);
      ...
      await this.logCommunication({
        ...
        recipient: options.recipientPhone,
        subject: options.template.subject,
        content: this.generateMessageContent(options.template, options.templateData),
        status: 'failed',
  ...
  async sendEventConfirmation(event: Event, userId: string): Promise<{
    success: boolean;
    messageId?: string;
    error?: string;
  }> {
    ...
    const template: SMSTemplate = {
      ...
      variables: ['customerName', 'eventType', 'datetime', 'location', 'companyName']
    };

    const templateData = {
      customerName: event.customer_name || 'Customer',
      eventType: this.formatEventType(event.event_type),
      datetime: this.formatDateTime(event.confirmed_datetime || event.proposed_datetime),
  ...
  async sendEventReminder(event: Event, userId: string, hoursBeforeEvent: number = 24): Promise<{
    success: boolean;
    messageId?: string;
    error?: string;
  }> {
    ...
    const template: SMSTemplate = {
      ...
      variables: ['eventType', 'companyName', 'timeUntil', 'datetime', 'location']
    };
  ...
  async sendEventUpdate(event: Event, userId: string, updateMessage: string): Promise<{
    success: boolean;
    messageId?: string;
    error?: string;
  }> {
  ...
  private generateMessageContent(template: SMSTemplate, data: Record<string, string>): string {
  ...
  private formatTimeUntilEvent(now: Date, eventDate: Date): string {
    const diffInHours = Math.round((eventDate.getTime() - now.getTime()) / (1000 * 60 * 60));
