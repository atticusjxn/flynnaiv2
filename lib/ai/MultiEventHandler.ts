...
import { ExtractedEvent, LiveExtractionResult } from '@/lib/ai/LiveEventExtractor';
import { classifyEvent, EventClassificationSystem } from '@/lib/ai/EventClassificationSystem';
...
export interface MultiEventAnalysis {
  ...
  recommendedAction: 'single_appointment' | 'multiple_appointments' | 'follow_up_call' | 'clarification_needed';
  prioritization: Array<{ event: ExtractedEvent; priority: number; reasoning: string }>;
  conflicts: Array<{ event1: ExtractedEvent; event2: ExtractedEvent; conflictType: string; resolution: string }>;
...
export class MultiEventHandler {
  ...
  public analyzeMultipleEvents(
    ...
    if (events.length === 1) {
      return this.createSingleEventAnalysis(events[0], transcriptionText, industry);
    }

    // Cluster related events
    const eventClusters = this.clusterEvents(events, transcriptionText, industry);
    ...
    const recommendedAction = this.determineRecommendedAction(eventClusters, conflicts, industry);
    
    // Calculate overall confidence
    const overallConfidence = this.calculateOverallConfidence(events, eventClusters);
   ...
   */
  private clusterEvents(events: ExtractedEvent[], transcriptionText: string, industry: string): EventCluster[] {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      for (let j = i + 1; j < events.length; j++) {
        ...
        const relationship = this.analyzeEventRelationship(primaryEvent, secondaryEvent, industry);
      ...
      const clusterType = this.determineClusterType(primaryEvent, relatedEvents, transcriptionText);
      
      // Calculate cluster confidence
      const confidence = this.calculateClusterConfidence(primaryEvent, relatedEvents);
      
      const reasoning = this.generateClusterReasoning(clusterType, primaryEvent, relatedEvents);
   ...
   */
  private analyzeEventRelationship(event1: ExtractedEvent, event2: ExtractedEvent, industry: string): {
    isRelated: boolean;
    relationshipType: 'duplicate' | 'sequence' | 'alternative' | 'complementary' | 'unrelated';
    confidence: number;
  } {
    
    // Check for duplicates (same event extracted multiple times)
    if (this.areEventsDuplicate(event1, event2)) {
      return { isRelated: true, relationshipType: 'duplicate', confidence: 0.9 };
    }
    ...
    if (this.areEventsAlternative(event1, event2)) {
      return { isRelated: true, relationshipType: 'alternative', confidence: 0.7 };
    }
    
    // Check for complementary services
    if (this.areEventsComplementary(event1, event2, industry)) {
      return { isRelated: true, relationshipType: 'complementary', confidence: 0.6 };
    }
   ...
   */
  private areEventsDuplicate(event1: ExtractedEvent, event2: ExtractedEvent): boolean {
    // Similar titles and descriptions
    const titleSimilarity = this.calculateStringSimilarity(event1.title, event2.title);
    const descriptionSimilarity = this.calculateStringSimilarity(event1.description, event2.description);
    
    // Same location and time
    const sameLocation = event1.location && event2.location && event1.location === event2.location;
    const sameTime = event1.proposed_datetime && event2.proposed_datetime && 
                    event1.proposed_datetime === event2.proposed_datetime;
    
    return (titleSimilarity > 0.8 || descriptionSimilarity > 0.8) && (sameLocation || sameTime);
  }

  /**
   * Check if events are sequential
   */
  private areEventsSequential(event1: ExtractedEvent, event2: ExtractedEvent, industry: string): boolean {
    const combinedDescription = `${event1.description} ${event2.description}`.toLowerCase();
    
    // Look for sequential indicators
    const sequentialKeywords = [
      'first', 'then', 'after', 'follow-up', 'second visit', 'next step',
      'initial', 'later', 'subsequent', 'return visit', 'check back'
    ];
    ...
    if (industry === 'plumbing') {
      const plumbingSequential = ['inspect', 'quote', 'install', 'follow-up', 'warranty'];
      return hasSequentialKeywords || plumbingSequential.some(keyword => 
        combinedDescription.includes(keyword)
      );
    }
    
    if (industry === 'medical') {
      const medicalSequential = ['consultation', 'test', 'results', 'treatment', 'follow-up'];
      return hasSequentialKeywords || medicalSequential.some(keyword => 
        combinedDescription.includes(keyword)
      );
    }
   ...
   */
  private areEventsAlternative(event1: ExtractedEvent, event2: ExtractedEvent): boolean {
    const combinedText = `${event1.title} ${event1.description} ${event2.title} ${event2.description}`.toLowerCase();
    
    const alternativeKeywords = [
      'or', 'either', 'maybe', 'option', 'choice', 'alternatively',
      'if not', 'otherwise', 'instead', 'preference'
    ];
    
    return alternativeKeywords.some(keyword => combinedText.includes(keyword));
  }

  /**
   * Check if events are complementary
   */
  private areEventsComplementary(event1: ExtractedEvent, event2: ExtractedEvent, industry: string): boolean {
    if (industry === 'plumbing') {
      const service1 = event1.service_type?.toLowerCase() || event1.description.toLowerCase();
      const service2 = event2.service_type?.toLowerCase() || event2.description.toLowerCase();
      
      // Common complementary plumbing services
      const complementaryPairs = [
        ['drain', 'pipe'], ['faucet', 'sink'], ['toilet', 'bathroom'],
        ['water heater', 'plumbing'], ['leak', 'repair']
      ];
  ...
  private determineClusterType(
    ...
    if (transcript.includes(' or ') || transcript.includes('either') || transcript.includes('maybe')) {
      return 'alternatives';
    }
    
    // Check for sequential language
    if (transcript.includes('first') || transcript.includes('then') || transcript.includes('after')) {
      return 'sequence';
    }
    
    // Check for combined service indicators
    if (transcript.includes('and also') || transcript.includes('while you\'re there') || 
   ...
   */
  private calculateClusterConfidence(primaryEvent: ExtractedEvent, relatedEvents: ExtractedEvent[]): number {
    const allEvents = [primaryEvent, ...relatedEvents];
    const totalConfidence = allEvents.reduce((sum, event) => sum + event.confidence, 0);
   ...
   */
  private prioritizeEvents(events: ExtractedEvent[], industry: string): Array<{ event: ExtractedEvent; priority: number; reasoning: string }> {
    const prioritized = events.map(event => {
      ...
      const contactScore = [event.customer_name, event.customer_phone, event.customer_email]
   ...
   * Detect conflicts between events
   */
  private detectEventConflicts(events: ExtractedEvent[]): Array<{ event1: ExtractedEvent; event2: ExtractedEvent; conflictType: string; resolution: string }> {
    const conflicts = [];
    ...
    for (let i = 0; i < events.length; i++) {
      for (let j = i + 1; j < events.length; j++) {
        ...
        // Time conflict
        if (event1.proposed_datetime && event2.proposed_datetime && 
            event1.proposed_datetime === event2.proposed_datetime) {
          conflicts.push({
            event1,
            event2,
            conflictType: 'timing',
            resolution: 'Schedule events at different times or combine if related'
          });
        }
        ...
        if (event1.customer_name && event2.customer_name && 
  ...
  private determineRecommendedAction(
    ...
    conflicts: any[], 
    industry: string
  ): 'single_appointment' | 'multiple_appointments' | 'follow_up_call' | 'clarification_needed' {
    
    if (clusters.length === 0) return 'clarification_needed';
    if (clusters.length === 1 && clusters[0].relatedEvents.length === 0) return 'single_appointment';
    ...
    // Check cluster types
    const hasAlternatives = clusters.some(c => c.clusterType === 'alternatives');
    if (hasAlternatives) return 'follow_up_call';
    
    const hasSequential = clusters.some(c => c.clusterType === 'sequence');
    const hasCombined = clusters.some(c => c.clusterType === 'combined_service');
   ...
   */
  private calculateOverallConfidence(events: ExtractedEvent[], clusters: EventCluster[]): number {
    if (events.length === 0) return 0;
    
    const eventConfidences = events.map(e => e.confidence);
    const averageEventConfidence = eventConfidences.reduce((sum, c) => sum + c, 0) / events.length;
    
    const clusterConfidences = clusters.map(c => c.confidence);
    const averageClusterConfidence = clusterConfidences.length > 0 
      ? clusterConfidences.reduce((sum, c) => sum + c, 0) / clusterConfidences.length
      : 1.0;
   ...
   */
  private createSingleEventAnalysis(event: ExtractedEvent, transcriptionText: string, industry: string): MultiEventAnalysis {
    ...
    const priority = {
      event,
      priority: event.urgency === 'emergency' ? 100 : 
               event.urgency === 'high' ? 75 : 
               event.urgency === 'medium' ? 50 : 25,
...
export function analyzeMultipleEvents(
  ...
  return multiEventHandler.analyzeMultipleEvents(events, transcriptionText, industry, callSid);
}