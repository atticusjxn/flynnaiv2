...
export interface ConfidenceResult {
  ...
  breakdown: { [factor: string]: { score: number; weight: number; reasoning: string } };
  recommendations: string[];
  qualityLevel: 'excellent' | 'good' | 'fair' | 'poor';
}

export class ConfidenceScoring {
  private industryWeights: { [industry: string]: { [factor: string]: number } } = {
    plumbing: {
      locationCompleteness: 0.25,    // Critical for service calls
      contactCompleteness: 0.20,     // Need to reach customer
      descriptionSpecificity: 0.15,  // Helps with preparation
      timeSpecificity: 0.15,         // Scheduling importance
      industryMatch: 0.10,           // Industry alignment
    ...
    real_estate: {
      locationCompleteness: 0.30,    // Property address essential
      contactCompleteness: 0.20,     // Customer contact critical
      timeSpecificity: 0.15,         // Showing scheduling
      industryMatch: 0.10,           // Buyer/seller context
      descriptionSpecificity: 0.10,  // Property details
  ...
  public calculateConfidence(
    ...
    const factors = this.calculateIndividualFactors(event, transcriptionText, transcriptionConfidence);
    
    // Get industry-specific weights
    const weights = this.industryWeights[industry] || this.industryWeights.general;
    
    // Calculate weighted overall confidence
    let overallConfidence = 0;
    const breakdown: { [factor: string]: { score: number; weight: number; reasoning: string } } = {};
    ...
    const recommendations = this.generateRecommendations(factors, event, industry);
  ...
  private calculateIndividualFactors(
    ...
    return {
      ...
      timeRealism: this.assessTimeRealism(event.proposed_datetime, event.urgency),
      ...
      descriptionSpecificity: this.assessDescriptionSpecificity(event.description, event.title),
      descriptionRealism: this.assessDescriptionRealism(event.description, event.type),
  ...
  private assessTimeSpecificity(proposedDateTime: string | null): number {
    ...
    if (dateTimeStr.includes('morning') || dateTimeStr.includes('afternoon') || dateTimeStr.includes('evening')) {
      if (dateTimeStr.match(/\d{4}-\d{2}-\d{2}/)) return 0.8;
    }
    
    // Relative specific time
    if (dateTimeStr.includes('tomorrow') && (dateTimeStr.includes('am') || dateTimeStr.includes('pm'))) return 0.9;
    
    // Relative date with general time
    if (dateTimeStr.includes('tomorrow') || dateTimeStr.includes('next week')) return 0.6;
    
    // Very general
    if (dateTimeStr.includes('soon') || dateTimeStr.includes('this week')) return 0.4;
   ...
   */
  private assessTimeRealism(proposedDateTime: string | null, urgency: string): number {
    ...
    try {
      if (dateTimeStr.match(/\d{4}-\d{2}-\d{2}/)) {
        ...
        if (dateMatch) {
          const proposedDate = new Date(parseInt(dateMatch[1]), parseInt(dateMatch[2]) - 1, parseInt(dateMatch[3]));
    ...
    if (urgency === 'emergency') {
      if (dateTimeStr.includes('now') || dateTimeStr.includes('asap') || dateTimeStr.includes('immediately')) return 1.0;
  ...
  private assessLocationCompleteness(location: string | null): number {
    ...
    if (loc.match(/\d+.*\b(street|st|avenue|ave|drive|dr|road|rd|lane|ln)\b.*\b(city|town)\b/)) return 1.0;
    
    // Street address with number
    if (loc.match(/\d+.*\b(street|st|avenue|ave|drive|dr|road|rd|lane|ln)\b/)) return 0.8;
    
    // Street name without number
    if (loc.match(/\b(street|st|avenue|ave|drive|dr|road|rd|lane|ln)\b/)) return 0.6;
    
    // General area/neighborhood
    if (loc.match(/\b(downtown|uptown|north|south|east|west|area|neighborhood)\b/)) return 0.4;
  ...
  private assessLocationRealism(location: string | null): number {
    ...
    if (loc.includes('123 fake') || loc.includes('nowhere') || loc.includes('n/a')) return 0.1;
    
    // Check for realistic patterns
    if (loc.match(/\d+\s+\w+\s+(street|st|avenue|ave|drive|dr|road|rd)/)) return 0.9;
  ...
  private assessContactCompleteness(event: ExtractedEvent): number {
    ...
    if (event.customer_name && event.customer_name.trim().length > 0) score += 1;
    if (event.customer_phone && event.customer_phone.match(/\d{10,}/)) score += 1;
  ...
  private assessContactRealism(event: ExtractedEvent): number {
    ...
    if (event.customer_name) {
      ...
      if (name.includes('john doe') || name.includes('test') || name.length < 3) {
        score += 0.2;
      } else {
        score += 0.9;
      }
    ...
    if (event.customer_email) {
      count++;
      if (event.customer_email.includes('@') && !event.customer_email.includes('test') && !event.customer_email.includes('example')) {
        score += 0.9;
      } else {
        score += 0.3;
      }
   ...
   */
  private assessDescriptionSpecificity(description: string, title: string): number {
    ...
    if (desc.length > 50 && (desc.includes('because') || desc.includes('when') || desc.includes('where'))) return 0.9;
   ...
   */
  private assessDescriptionRealism(description: string, eventType: string): number {
    if (!description) return 0.5;
    
    const desc = description.toLowerCase();
    
    // Check for generic/template descriptions
    if (desc.includes('lorem ipsum') || desc.includes('test description') || desc.includes('placeholder')) return 0.1;
    ...
    if (eventType === 'quote' && !desc.includes('price') && !desc.includes('cost') && !desc.includes('estimate')) return 0.6;
   ...
   */
  private assessIndustryMatch(event: ExtractedEvent, transcriptionText: string): number {
    ...
    const plumbingKeywords = ['plumb', 'pipe', 'leak', 'drain', 'water', 'toilet', 'sink', 'faucet'];
    const realEstateKeywords = ['house', 'property', 'real estate', 'buy', 'sell', 'show', 'listing'];
    const legalKeywords = ['legal', 'lawyer', 'attorney', 'court', 'lawsuit', 'consultation'];
    const medicalKeywords = ['doctor', 'appointment', 'medical', 'health', 'symptoms', 'patient'];
    
    const keywordSets = [plumbingKeywords, realEstateKeywords, legalKeywords, medicalKeywords];
    let maxMatches = 0;
   ...
   */
  private assessTerminologyMatch(event: ExtractedEvent, transcriptionText: string): number {
  ...
  private assessExtractionConsistency(event: ExtractedEvent): number {
    ...
    if (event.proposed_datetime && event.urgency === 'emergency') {
      ...
      if (!dateStr.includes('now') && !dateStr.includes('asap') && !dateStr.includes('immediately')) {
   ...
   */
  private getFactorReasoning(factorName: string, score: number, event: ExtractedEvent): string {
    const scoreCat = score > 0.8 ? 'excellent' : score > 0.6 ? 'good' : score > 0.4 ? 'fair' : 'poor';
   ...
   */
  private generateRecommendations(factors: ConfidenceFactors, event: ExtractedEvent, industry: string): string[] {
    const recommendations: string[] = [];
    
    if (factors.timeSpecificity < 0.5) {
      recommendations.push('Try to capture more specific timing information from the conversation');
    }
    
    if (factors.locationCompleteness < 0.6 && industry === 'plumbing') {
      recommendations.push('Service address is critical for plumbing calls - listen for street names or landmarks');
    }
    
    if (factors.contactCompleteness < 0.7) {
      recommendations.push('Focus on capturing customer name and phone number for follow-up');
    }
    
    if (factors.descriptionSpecificity < 0.6) {
      recommendations.push('Extract more detailed description of what service is needed');
    }
    
    if (factors.extractionConsistency < 0.8) {
      recommendations.push('Review extraction for logical consistency between urgency, type, and details');
    }
...
export function calculateEventConfidence(
  ...
  return confidenceScoring.calculateConfidence(event, transcriptionText, industry, transcriptionConfidence);
}