...
export class AccuracyTesting {
   ...
   */
  public async testIndustryAccuracy(
    industry: string
  ): Promise<IndustryAccuracy> {
    const industryTests = this.testCases.filter(
      (test) => test.industry === industry
    );
    const results: AccuracyResult[] = [];

    console.log(
      `Running ${industryTests.length} tests for ${industry} industry...`
    );
    ...
    for (const testCase of industryTests) {
      ...
      results.push(result);
      console.log(
        `Test ${testCase.id}: ${result.passed ? 'PASSED' : 'FAILED'} (${(result.accuracy * 100).toFixed(1)}% accuracy)`
      );
    }
  ...
  public async runAllAccuracyTests(): Promise<{
    ...
    const totalTests = industryResults.reduce(
      (sum, r) => sum + r.totalTests,
      0
    );
    const totalPassed = industryResults.reduce(
      (sum, r) => sum + r.passedTests,
      0
    );
    const overallAccuracy =
      industryResults.reduce(
        (sum, r) => sum + r.averageAccuracy * r.totalTests,
        0
      ) / totalTests;
    const averageProcessingTime =
      industryResults.reduce(
        (sum, r) => sum + r.averageProcessingTime * r.totalTests,
        0
      ) / totalTests;

    const meetsTarget = overallAccuracy >= 0.9; // 90% accuracy target
  ...
  private async runSingleTest(testCase: TestCase): Promise<AccuracyResult> {
    ...
    try {
      ...
      const passed = accuracy >= 0.8; // 80% threshold for individual tests
  ...
  private calculateTestAccuracy(
    ...
    if (actualCount === expectedCount) {
      ...
      const countAccuracy =
        Math.min(actualCount, expectedCount) /
        Math.max(actualCount, expectedCount);
    ...
    for (let i = 0; i < expectedCount; i++) {
      ...
      if (bestMatch) {
        ...
        if (expected.urgency !== bestMatch.urgency) {
          issues.push(
            `Incorrect urgency: expected ${expected.urgency}, got ${bestMatch.urgency}`
          );
  ...
  private scoreEventMatch(expected: any, extracted: ExtractedEvent): number {
    ...
    if (
      ...
      (expected.type === 'service_call' &&
        ['repair', 'installation', 'maintenance'].includes(extracted.type))
    ) {
      score += 30;
    ...
    const titleMatch = this.calculateStringRelevance(
      expected.title,
      extracted.title
    );
    const descMatch = this.calculateStringRelevance(
      expected.title,
  ...
  private isRelatedUrgency(expected: string, actual: string): boolean {
    ...
    return urgencyGroups.some(
      (group) => group.includes(expected) && group.includes(actual)
    );
  }
...
export async function testIndustryAccuracy(
  industry: string
): Promise<IndustryAccuracy> {
