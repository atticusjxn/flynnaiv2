...
export class EventClassificationSystem {
  private industryRules: { [industry: string]: IndustryClassificationRules } = {
    plumbing: {
      industry: 'plumbing',
      primaryTypes: ['service_call', 'emergency', 'quote', 'installation', 'repair', 'maintenance'],
      keywords: {
        service_call: ['fix', 'repair', 'come out', 'look at', 'check', 'service'],
        emergency: ['flooding', 'burst pipe', 'no water', 'leak', 'urgent', 'emergency'],
        ...
        maintenance: ['tune-up', 'maintenance', 'service', 'check-up', 'annual']
      },
      urgencyModifiers: {
        'flooding': 2.0,
        'emergency': 1.8,
      ...
      emergencyTriggers: ['flooding', 'burst pipe', 'no water', 'sewage', 'gas leak']
    },

    real_estate: {
      industry: 'real_estate',
      primaryTypes: ['showing', 'meeting', 'listing_consultation', 'buyer_consultation', 'closing'],
      keywords: {
        showing: ['show', 'tour', 'see property', 'walk through', 'visit'],
        listing_consultation: ['list', 'sell', 'market value', 'CMA', 'appraisal'],
      ...
      defaultType: 'showing',
      emergencyTriggers: ['closing tomorrow', 'offer deadline', 'contract expiring']
    },

    legal: {
      industry: 'legal',
      primaryTypes: ['legal_consultation', 'meeting', 'court_appearance', 'consultation'],
      keywords: {
      ...
      emergencyTriggers: ['court', 'arrested', 'deadline', 'statute of limitations']
    },

    medical: {
      industry: 'medical',
      primaryTypes: ['medical_appointment', 'urgent_care', 'procedure', 'wellness_visit', 'follow_up'],
      keywords: {
      ...
      emergencyTriggers: ['chest pain', 'breathing', 'bleeding', 'overdose', 'stroke']
    },
  ...
  public classifyEvent(
    ...
    const typeScores: Array<{ type: EventType; score: number; matches: string[] }> = [];
    ...
    for (const eventType of rules.primaryTypes) {
      ...
      for (const [modifier, multiplier] of Object.entries(rules.urgencyModifiers)) {
        if (description.includes(modifier.toLowerCase()) || 
    ...
    if (typeScores.length > 0 && typeScores[0].score > 0) {
      ...
      const baseConfidence = Math.min(typeScores[0].score / maxPossibleScore, 1.0);
  ...
  public validateClassification(
    ...
    if (classification.eventType === 'emergency' && extractedEvent.urgency !== 'emergency') {
      issues.push('Event classified as emergency but urgency level is not emergency');
      recommendations.push('Consider updating urgency level to match emergency classification');
    }
    
    // Check confidence vs extracted confidence
    if (classification.confidence > 0.8 && extractedEvent.confidence < 0.6) {
      issues.push('Classification confidence high but extraction confidence low');
    ...
    if (rules && !rules.primaryTypes.includes(classification.eventType)) {
      issues.push(`Event type ${classification.eventType} not common for ${rules.industry} industry`);
      recommendations.push('Consider alternative classification or review industry context');
    }
  ...
  public getClassificationStats(classifications: ClassificationResult[]): {
    ...
    const averageConfidence = classifications.reduce((sum, c) => sum + c.confidence, 0) / totalClassifications;
    ...
    for (const classification of classifications) {
      // Type distribution
      typeDistribution[classification.eventType] = (typeDistribution[classification.eventType] || 0) + 1;
      
      // Industry breakdown
      industryBreakdown[classification.industryContext] = (industryBreakdown[classification.industryContext] || 0) + 1;
...
export function classifyEvent(
  ...
  return eventClassificationSystem.classifyEvent(extractedEvent, transcriptionText, industry);
}