...
export function useOptimisticUpdates<T = any>() {
  ...
  const applyOptimisticUpdatesToData = useCallback(<TData extends { id?: string }>(
    originalData: TData[],
    table: string
  ): TData[] => {
    ...
    for (const update of tableUpdates) {
      switch (update.type) {
        ...
        case 'delete':
          result = result.filter(item => item.id !== (update.data as any).id)
          break
      }
    }
  ...
  const optimisticUpdate = useCallback(async (
    ...
    const updateId = applyOptimisticUpdate('update', table, { ...updates, id } as T, originalData)
  ...
  const optimisticDelete = useCallback(async (
    ...
    const updateId = applyOptimisticUpdate('delete', table, { id } as T, originalData)
  ...
  const clearFailedUpdates = useCallback(() => {
    setPendingUpdates(prev => prev.filter(update => update.status !== 'failed'))
  }, [])
...
export function useOptimisticEvents() {
  ...
  const updateEventStatus = useCallback(async (
    ...
    return optimisticUpdate('events', eventId, { 
      status: newStatus,
      updated_at: new Date().toISOString()
    }, originalEvent)
  }, [optimisticUpdate])

  const confirmEvent = useCallback(async (eventId: string, originalEvent?: any) => {
    return updateEventStatus(eventId, 'confirmed', originalEvent)
  }, [updateEventStatus])

  const cancelEvent = useCallback(async (eventId: string, originalEvent?: any) => {
