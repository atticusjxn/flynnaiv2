...
export const useAdvancedSearch = (options: UseAdvancedSearchOptions = {}) => {
  ...
  const performSearch = useCallback(async (filters: SearchFilters): Promise<SearchResult[]> => {
    ...
    try {
      ...
      if (filters.categories.includes('calls')) {
        ...
        if (filters.aiFilters.minConfidence > 0) {
          callsQuery = callsQuery.gte('transcription_confidence', filters.aiFilters.minConfidence / 100);
        }
        ...
        if (callsError) {
          ...
          const callResults: SearchResult[] = callsData.map((call) => ({
            id: call.id,
            type: 'call' as const,
            title: call.main_topic || `Call from ${call.caller_name || call.caller_number}`,
            description: call.call_summary || call.transcription_text?.substring(0, 200) + '...' || 'No summary available',
            metadata: {
              ...
              confidence: call.transcription_confidence ? Math.round(call.transcription_confidence * 100) : undefined,
              callId: call.id,
            },
            highlights: generateHighlights(filters.query, call, 'call'),
            relevanceScore: calculateRelevanceScore(filters.query, call, 'call'),
          }));
      ...
      if (filters.categories.includes('events')) {
        ...
        if (filters.urgencyLevels.length > 0) {
          eventsQuery = eventsQuery.in('urgency_level', filters.urgencyLevels);
        }

        // Apply customer filters
        if (filters.customerFilters.hasContactInfo) {
          eventsQuery = eventsQuery.or('customer_name.not.is.null,customer_phone.not.is.null,customer_email.not.is.null');
        }
        ...
        if (filters.locationFilters.locationType.length > 0) {
          eventsQuery = eventsQuery.in('location_type', filters.locationFilters.locationType);
        }
        ...
        if (eventsError) {
          ...
          const eventResults: SearchResult[] = eventsData.map((event) => ({
            ...
            type: 'event' as const,
            title: event.title,
            description: event.description || `${event.event_type} event${event.customer_name ? ` with ${event.customer_name}` : ''}`,
            metadata: {
              date: event.proposed_datetime || event.confirmed_datetime || event.created_at,
            ...
            relevanceScore: calculateRelevanceScore(filters.query, event, 'event'),
          }));
  ...
  const generateHighlights = useCallback((query: string, data: any, type: 'call' | 'event') => {
    if (!query.trim()) return [];

    const highlights: { field: string; snippet: string; }[] = [];
    const searchTerms = query.toLowerCase().split(/\s+/);

    const searchFields = type === 'call' 
    ...
    searchFields.forEach(field => {
      ...
      if (typeof value === 'string' && value.trim()) {
        ...
        const matchingTerms = searchTerms.filter(term => lowerValue.includes(term));
        ...
        if (matchingTerms.length > 0) {
          ...
          if (startIndex + maxSnippetLength < value.length) snippet = snippet + '...';
  ...
  const calculateRelevanceScore = useCallback((query: string, data: any, type: 'call' | 'event'): number => {
      ...
      if (typeof value === 'string' && value.trim()) {
        ...
        searchTerms.forEach(term => {
          if (lowerValue.includes(term)) {
            ...
            const exactMatches = (lowerValue.match(new RegExp(`\\b${term}\\b`, 'g')) || []).length;
            const partialMatches = (lowerValue.match(new RegExp(term, 'g')) || []).length - exactMatches;
            
            score += (exactMatches * weight * 1.0) + (partialMatches * weight * 0.5);
          }
        });
      }
    });
    ...
    if (type === 'call') {
      if (data.transcription_confidence && data.transcription_confidence > 0.8) bonus += 0.1;
      if (data.call_summary && data.call_summary.length > 50) bonus += 0.05;
      if (data.urgency_level === 'emergency' || data.urgency_level === 'high') bonus += 0.05;
    } else if (type === 'event') {
  ...
  const getSuggestions = useCallback(async (query: string): Promise<SearchSuggestion[]> => {
    ...
    try {
      ...
      const { data: recentEvents } = await supabase
        ...
        .eq('user_id', userId)
        .or(`title.ilike.%${query}%,customer_name.ilike.%${query}%,location.ilike.%${query}%`)
        .order('created_at', { ascending: false })
        .limit(5);

      if (recentEvents) {
        recentEvents.forEach(event => {
          if (event.customer_name?.toLowerCase().includes(query.toLowerCase())) {
            suggestions.push({
  ...
  // Save a search query
  const saveSearch = useCallback(async (search: Omit<SavedSearch, 'id' | 'createdAt' | 'lastUsed' | 'useCount'>) => {
    try {
  ...
  const exportResults = useCallback(async (results: SearchResult[], format: 'csv' | 'excel' | 'json') => {
    try {
      ...
      switch (format) {
        ...
        case 'excel':
          ...
          const excelContent = [
            excelHeaders.join('\t'),
            ...dataToExport.map(row => 
              excelHeaders.map(header => String(row[header as keyof typeof row] || '')).join('\t')
            )
          ].join('\n');
