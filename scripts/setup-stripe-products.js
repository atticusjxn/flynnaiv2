...
const PRODUCTS_TO_CREATE = [
  {
    name: 'Flynn.ai Basic',
    description: 'AI call notes and event extraction with professional email delivery',
    features: [
  ...
  },
  {
    ...
    description: 'Advanced calendar sync with SMS notifications and bulk management',
    features: [
      'Everything in Basic',
...
async function createProducts() {
  try {
    ...
    for (const product of PRODUCTS_TO_CREATE) {
      ...
      console.log(`   Price: $${product.price_aud} AUD/${product.billing_period}`);
      ...
      try {
        ...
        console.log(`   ⚠️  Note: Manual creation required via Stripe Dashboard`);
        console.log(`   Error: ${error.message}`);
      }
    }
    
    console.log('\n📝 Next Steps:');
    console.log('1. Create products manually in Stripe Dashboard if MCP creation failed');
