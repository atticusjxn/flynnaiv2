...
const formatIndustryName = (industryType: string) => {
  ...
  if (industryType in AUSTRALIAN_INDUSTRY_CONFIG) {
    return AUSTRALIAN_INDUSTRY_CONFIG[industryType as keyof typeof AUSTRALIAN_INDUSTRY_CONFIG].displayName;
  }
...
const getIndustryContext = (industryType: string) => {
  if (industryType in AUSTRALIAN_INDUSTRY_CONFIG) {
    const config = AUSTRALIAN_INDUSTRY_CONFIG[industryType as keyof typeof AUSTRALIAN_INDUSTRY_CONFIG];
    return {
      businessHours: config.businessHours,
      averageRate: config.australianContext.averageRate || config.australianContext.averageCommission,
      peakSeason: config.australianContext.peakSeason,
                ...
                <div className="group relative">
                  ...
                  {getIndustryContext(profile.industry_type) && (
                    <div className="absolute top-full left-0 mt-2 p-4 bg-white border border-gray-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-20 min-w-80">
                      {(() => {
                        const context = getIndustryContext(profile.industry_type);
                        return context ? (
                          <div className="space-y-2 text-xs">
                            <div className="flex justify-between">
                              <span className="text-gray-600">Business Hours:</span>
                              <span className="font-medium text-gray-800">{context.businessHours}</span>
                            </div>
                            {context.averageRate && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Average Rate:</span>
                                <span className="font-medium text-gray-800">{context.averageRate}</span>
                              </div>
                            )}
                            <div className="flex justify-between">
                              <span className="text-gray-600">Peak Season:</span>
                              <span className="font-medium text-gray-800">{context.peakSeason}</span>
                            </div>
                            <div className="pt-1 border-t border-gray-100">
                              <span className="text-gray-600">Regulations:</span>
                              <p className="font-medium text-gray-800 mt-1">{context.regulations}</p>
                            </div>
                          </div>
                        ) : null;
                      })()}
            ...
            <div className="relative z-10">
              ...
              <div className="pt-6 border-t border-border/50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-sm font-bold text-primary">3/5 Complete</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner overflow-hidden">
                  <div className="bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 h-3 rounded-full transition-all duration-700 ease-out shadow-sm" 
                       style={{ width: '60%' }}>
                    <div className="h-full bg-gradient-to-r from-white/20 to-transparent rounded-full"></div>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-3 leading-relaxed">
                  Complete your Twilio and calendar integration to unlock full automation potential.
                </p>
              </div>
            </div>
          </div>
