...
export default function BillingPage() {
  ...
  useEffect(() => {
    async function fetchTrialStatus() {
      ...
      try {
        const response = await fetch(`/api/billing/trial-status?userId=${user.id}`);
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="bg-card border border-red-200 rounded-2xl shadow-sm p-12 text-center">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Loading Error</h2>
  ...
  const isOnTrial = trialStatus?.subscriptionStatus === 'trial' && trialStatus.isTrialActive;
  const hasActiveSubscription = ['active', 'past_due', 'incomplete'].includes(trialStatus?.subscriptionStatus || '');
    ...
    <div className="space-y-8">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-green-700 text-sm">Your subscription is now active. Welcome to Flynn.ai!</p>
                </div>
              </div>
            </motion.div>
          )}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-amber-700 text-sm">No worries! Your trial is still active. You can upgrade anytime.</p>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <div className="grid gap-8">
        {isOnTrial ? (
          ...
          <SubscriptionDashboard
            subscriptionStatus={trialStatus?.subscriptionStatus as 'active' | 'past_due' | 'cancelled' | 'incomplete'}
          ...
          <div className="bg-card border border-border rounded-2xl shadow-sm p-8 text-center">
            <h2 className="text-xl font-bold text-foreground mb-4">Start Your Flynn.ai Journey</h2>
        ...
        >
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              Start with a 30-day free trial, then continue with the plan that fits your needs
            </p>
          </div>
          
          <SubscriptionTierSelector
            currentTier={trialStatus?.subscriptionStatus === 'active' ? selectedTier : undefined}
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/10 dark:to-blue-950/10 rounded-lg p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-foreground mb-2">🇦🇺 Built for Australian Businesses</h3>
            <p className="text-sm text-muted-foreground">
              GST inclusive pricing • Local support available • Optimized for AEST/AEDT timezone
            </p>
          </div>
          <div className="text-right">
            <div className="text-sm font-medium text-foreground">Need help?</div>
            <div className="text-sm text-muted-foreground">support@flynn.ai</div>
          </div>
        </div>
      </div>
    </div>
