...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: featureEvents, error: eventsError } = await query
    ...
    const featureStats = featureEvents?.reduce((acc, event) => {
      ...
      acc[feature].byIndustry[industry] = (acc[feature].byIndustry[industry] || 0) + 1;
      acc[feature].byTier[tier] = (acc[feature].byTier[tier] || 0) + 1;
      acc[feature].dailyUsage[date] = (acc[feature].dailyUsage[date] || 0) + 1;

      return acc;
    }, {} as Record<string, any>) || {};

    // Convert Sets to counts and format data
    const processedStats = Object.entries(featureStats).map(([feature, stats]) => ({
      feature,
      totalUsage: stats.totalUsage,
      uniqueUsers: stats.uniqueUsers.size,
      adoptionRate: stats.uniqueUsers.size / (featureEvents?.length || 1) * 100,
    ...
    const adoptionHeatMap = processedStats.reduce((acc, stat) => {
      acc[stat.feature] = {
    ...
    const usageTrends = processedStats.reduce((acc, stat) => {
    ...
    if (validatedParams.user_id === user.id) {
      const userEvents = featureEvents?.filter(event => event.user_id === user.id) || [];
      userUsage = {
        totalFeatureUsage: userEvents.length,
        featuresUsed: [...new Set(userEvents.map(e => e.event_name))].length,
        mostUsedFeatures: userEvents.reduce((acc, event) => {
    ...
    return NextResponse.json({
      summary: {
        ...
        averageUsagePerUser: featureEvents?.length ? 
          featureEvents.length / new Set(featureEvents.map(e => e.user_id)).size : 0,
      },
      topFeatures,
      adoptionHeatMap,
      usageTrends,
