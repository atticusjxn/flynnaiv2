...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(
          `/settings?calendar_error=${encodeURIComponent(error)}`,
          request.url
        )
      );
    }
    ...
    const tokenResult = await googleCalendarAuth.exchangeCodeForTokens(
      code,
      state
    );
    ...
    if (!tokenResult.success || !tokenResult.tokens) {
      ...
      return NextResponse.redirect(
        new URL(
          `/settings?calendar_error=${encodeURIComponent(tokenResult.error || 'token_exchange_failed')}`,
          request.url
        )
      );
    }
    ...
    const { error: dbError } = await supabase
      ...
      .upsert({
        ...
        expires_at: tokenResult.tokens.expiry_date
    ...
    console.log(
      'Successfully stored Google Calendar integration for user:',
      userId
    );

    // Test the integration by fetching calendar list
    try {
      const client = googleCalendarAuth.getAuthenticatedClient(
        tokenResult.tokens
      );
      ...
      console.log(
        `Successfully connected to ${calendarList.data.items?.length || 0} calendars`
      );
    ...
    return NextResponse.redirect(
      new URL(
        `/settings?calendar_error=${encodeURIComponent('callback_processing_failed')}`,
        request.url
      )
    );
  }
