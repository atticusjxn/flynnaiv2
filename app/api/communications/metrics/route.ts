...
export async function GET(request: NextRequest) {
  try {
    ...
    const metrics = await communicationLogger.getCommunicationMetrics(
      user.id,
      dateFrom,
      dateTo
    );
    ...
    const failedCommunications =
      await communicationLogger.getFailedCommunications(user.id, 24);
...
async function getResponseRates(
  userId: string,
  dateFrom?: Date,
  dateTo?: Date
) {
