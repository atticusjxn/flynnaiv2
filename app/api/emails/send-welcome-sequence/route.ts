...
export async function POST(request: NextRequest) {
  try {
    const { userId, industry, phoneNumber, emailType = 'welcome' } = await request.json();
    ...
    switch (emailType) {
      case 'welcome':
        ...
        break;
      case 'setup_complete':
        emailResult = await sendSetupCompleteEmail(user, userIndustry, phoneNumber);
        break;
      case 'sequence':
...
async function sendSetupCompleteEmail(user: any, industry: string, phoneNumber?: string) {
  const dashboardUrl = `${process.env.NEXT_PUBLIC_APP_URL}/dashboard`;
  const supportUrl = `${process.env.NEXT_PUBLIC_APP_URL}/support`;

  const emailHtml = render(SetupCompleteEmail({
    userName: user.full_name || 'there',
    companyName: user.company_name || 'Your Business',
    industry: industry,
