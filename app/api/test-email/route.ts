...
import { InstantEmailService, EmailDeliveryRequest } from '@/lib/email/InstantEmailService';
  ...
  try {
    ...
    if (action === 'test-send') {
      ...
      const testEmailRequest: EmailDeliveryRequest = {
        ...
        callDuration: 145,
        transcriptionText: 'Hi, this is John calling about getting my kitchen sink fixed. I have a leak under the sink that\'s getting worse. Could someone come out tomorrow afternoon around 2 PM? My address is 123 Main Street. My phone number is 555-1234. It\'s not an emergency but I\'d like to get it fixed soon.',
        extractedEvents: [
          {
            id: 'event_test_1',
            type: 'service_call',
            ...
            proposed_date: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Tomorrow
            proposed_time: '14:00',
            duration_minutes: 90,
            urgency: 'medium',
            customer_name: 'John',
            ...
            extraction_notes: 'Clear service request with specific time, location, and contact info'
          }
        ]
      };
