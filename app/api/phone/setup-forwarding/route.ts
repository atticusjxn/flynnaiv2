...
export async function POST(request: NextRequest) {
  try {
    ...
    const forwardingSetup = await setupCallForwarding(userPhoneNumber, preferredAreaCode);
    ...
    await sendForwardingInstructions(userPhoneNumber, forwardingSetup.flynnNumber);
...
async function setupCallForwarding(userPhoneNumber: string, preferredAreaCode?: string) {
  try {
    // Search for available phone numbers
    const availableNumbers = await client.availablePhoneNumbers('AU').local.list({
    ...
    if (availableNumbers.length === 0) {
      ...
      const fallbackNumbers = await client.availablePhoneNumbers('AU').local.list({ limit: 1 });
...
async function sendForwardingInstructions(userPhoneNumber: string, flynnNumber: string) {
  try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('forwarding_number, user_phone_number, forwarding_setup_complete, ai_processing_enabled')
      .eq('id', user.id)
      .single();
    ...
    return NextResponse.json({
      ...
      forwardingCode: data?.forwarding_number ? generateForwardingCode(data.forwarding_number) : null
    });
