...
export default function DebugEnv() {
  ...
  const debugInfo = {
    'window.location.origin': typeof window !== 'undefined' ? window.location.origin : 'SSR - not available',
    'Current URL': typeof window !== 'undefined' ? window.location.href : 'SSR - not available',
  };
          ...
          <div className="space-y-2">
            {Object.entries(debugInfo).map(([key, value]) => (
              <div key={key} className="flex justify-between text-xs">
                ...
                <span className="text-gray-600 truncate ml-2" title={String(value)}>
                  {String(value)}
                </span>
              </div>
            ))}
